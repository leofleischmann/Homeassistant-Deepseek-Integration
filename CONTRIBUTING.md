...
3. Ensure [CI](.github/workflows/ci.yml) passes (HACS validation + hassfest + unit tests).
   Run the unit tests locally with `python -m unittest discover -s tests` — they need no
   Home Assistant install. They cover the modules whose logic is plain Python: `const`,
   `models`, `options`, `request_builder`, `markdown_strip`, and the two delta helpers
   in `stream_transform`.
